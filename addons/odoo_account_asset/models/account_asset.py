...
class AccountAssetCategory(models.Model):
    _name = 'account.asset.category.custom'
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True, default=lambda self: self.env['res.company']._company_default_get('account.asset.category.custom'))
...
class AccountAssetAsset(models.Model):
    _name = 'account.asset.asset.custom'
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True, readonly=True, states={'draft': [('readonly', False)]},
        default=lambda self: self.env['res.company']._company_default_get('account.asset.asset.custom'))
    note = fields.Text()
    category_id = fields.Many2one('account.asset.category.custom', string='Category', required=True, change_default=True, readonly=True, states={'draft': [('readonly', False)]})
    date = fields.Date(string='Date', required=True, readonly=True, states={'draft': [('readonly', False)]}, oldname="purchase_date")
    ...
    depreciation_line_ids = fields.One2many('account.asset.depreciation.line.custom', 'asset_id', string='Depreciation Lines', readonly=True, states={'draft': [('readonly', False)], 'open': [('readonly', False)]})
    ...
    def _get_last_depreciation_date(self):
        """
        @param id: ids of a account.asset.asset.custom objects
        @return: Returns a dictionary of the effective dates of the last depreciation entry made for given asset ids. If there isn't any, return the purchase date of this asset
        """
        self.env.cr.execute("""
            SELECT a.id as id, COALESCE(MAX(m.date),a.date) AS date
            FROM account_asset_asset_custom a
            LEFT JOIN account_asset_depreciation_line_custom rel ON (rel.asset_id = a.id)
    ...
    def compute_generated_entries(self, date, asset_type=None):
        ...
        ungrouped_assets = self.env['account.asset.asset.custom'].search(type_domain + [('state', '=', 'open'), ('category_id.group_entries', '=', False)])
        ...
        for grouped_category in self.env['account.asset.category.custom'].search(type_domain + [('group_entries', '=', True)]):
            assets = self.env['account.asset.asset.custom'].search([('state', '=', 'open'), ('category_id', '=', grouped_category.id)])
                ...
                if self.prorata:
                    ...
                    if sequence == 1:
                        if self.method_period % 12 != 0:
                            ...
                            date = self.date #odoo12 no need to convert
        ...
        if self.value_residual != 0.0:
            ...
            if self.prorata:
                ...
                if posted_depreciation_line_ids and posted_depreciation_line_ids[-1].depreciation_date:
                    ...
                    depreciation_date = datetime.strptime(str(self._get_last_depreciation_date()[self.id]), DF).date()
            else:
                ...
                if self.method_period >= 12:
                    if self.company_id.fiscalyear_last_month:
                        ...
                        # print("year.......cfvgbhnj",str(self.date)[:4])
                        # w = date(year=int(str(self.date)[:4]))
                        # print("##############",w)
                        # # year=int(self.date[:4])
                        # # print("year............",year)
                        # p = relativedelta(year=time.strftime("%Y"))
                        # print("YEAR@@@@@@@@@@@",p)
                        # print("@@@@@@@@@@@@@@@@@@@@@@@@",self.company_id.fiscalyear_last_month)
                        # # n = relativedelta(year=int(self.date[:4]))
                        # # print("N..................",n)
                        asset_date = date(year=int(str(self.date)[:4]),
                                                   month=self.company_id.fiscalyear_last_month,
                                                   day=self.company_id.fiscalyear_last_day) + \
                                     relativedelta(days=1) + \
                                     relativedelta(year=int(str(self.date)[:4]))
                                     # relativedelta(year=time.strftime("%Y"))
                                      # e.g. 2018-12-31 +1 -> 2019
                    else:
                        # asset_date = datetime.strptime(str(self.date[:4] + '-01-01'), DF).date()
                        asset_date = datetime.strptime(str(self.date.strftime("%Y-%m-%d")[:4] + '-01-01'), DF).date()
                else:
                    # asset_date = datetime.strptime(self.date[:7] + '-01', DF).date()
                    asset_date = datetime.strptime(self.date.strftime("%Y-%m-%d")[:7] + '-01', DF).date()
                # if we already have some previous validated entries, starting date isn't 1st January but last entry + method period
                if posted_depreciation_line_ids and posted_depreciation_line_ids[-1].depreciation_date:
                    print ("self.method_period:::::::::::",self.method_period)
                    print ("posted_depreciation_line_ids....",posted_depreciation_line_ids)
                    # last_depreciation_date = datetime.strptime(posted_depreciation_line_ids[-1].depreciation_date, DF).date()
                    last_depreciation_date = posted_depreciation_line_ids[-1].depreciation_date
                    
                    print ("last_depreciation_date:::::::",last_depreciation_date)
    ...
    def validate(self):
        ...
        ref_tracked_fields = self.env['account.asset.asset.custom'].fields_get(fields)
        for asset in self:
    ...
    def _get_disposal_moves(self):
        ...
        for asset in self:
            ...
            if unposted_depreciation_line_ids:
                ...
                tracked_fields = self.env['account.asset.asset.custom'].fields_get(['method_number', 'method_end'])
    ...
    def _entry_count(self):
        for asset in self:
            res = self.env['account.asset.depreciation.line.custom'].search_count([('asset_id', '=', asset.id), ('move_id', '!=', False)])
    ...
    def onchange_category_id_values(self, category_id):
        if category_id:
            category = self.env['account.asset.category.custom'].browse(category_id)
            return {
                'value': {
    ...
    def _compute_entries(self, date, group_entries=False):
        depreciation_ids = self.env['account.asset.depreciation.line.custom'].search([
...
class AccountAssetDepreciationLine(models.Model):
    _name = 'account.asset.depreciation.line.custom'
    _description = 'Asset depreciation line'

    name = fields.Char(string='Depreciation Name', required=True, index=True)
    sequence = fields.Integer(required=True)
    asset_id = fields.Many2one('account.asset.asset.custom', string='Asset', required=True, ondelete='cascade')
